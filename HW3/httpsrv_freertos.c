...
static int cgi_temperature(HTTPSRV_CGI_REQ_STRUCT *param);
...
static int cgi_example(HTTPSRV_CGI_REQ_STRUCT *param)
{
	...
	PRINTF("Temperature from http_freertos.c as INTEGER: %d\r\n", temp);

	sprintf(cgi_data, "%d", temp);
...
static int cgi_temperature(HTTPSRV_CGI_REQ_STRUCT *param)
{
    ...
    response.status_code = HTTPSRV_CODE_OK;
    response.content_type = HTTPSRV_CONTENT_TYPE_PLAIN;  // Plaintext content type
    response.data = temp_str;  // Set the response data to the temperature string
    response.data_length = strlen(temp_str);  // Set data length based on the string length
    response.content_length = response.data_length;  // Set the content length
